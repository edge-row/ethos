...
async def start_bot():
    ...
    async def checkin(interaction: discord.Interaction):
        ...
        if user_id not in user_checkins:
            user_checkins[user_id] = {'username': interaction.user.display_name, 'last_chckin': None, 'yearly_count': 0}
    ...
    @bot.tree.command(name="yearly_check_in", description="See your total check ins this year.")
    async def yearly_checkins(interaction: discord.Interaction):
        user_id = str(interaction.user.id)
        if user_id not in user_checkins:
            await interaction.response.send_message(f"{interaction.user.mention} You have no check-ins.")
        else:
            await interaction.response.send_message(
    ...
    async def leaderboard(interaction: discord.Interaction):
        sorted_checkins = sorted(user_checkins.items(), key=lambda x: x[1]['yearly_count'], reverse=True)
        leaderboard_message = "Leaderboard:\n"
        for i, (user_id, data) in enumerate(sorted_checkins, start=1):
            leaderboard_message += f"{i}. {data['username']} - {data['yearly_count']} check-ins\n"
